...
# Eval(Lambda, pp, x0, t)
def eval(Dtree, root, string, pp, t):
    data = Dtree.tree.get_node(root).data
    data['y'], data['pai'] = uVDF.Eval(Lambda, pp, data['x'], t)
...
def vfTree(DtreeVf, Dtree, root, k, string, pp):
    if DtreeVf.getChildrenID(DtreeVf.getChildrenID(root, k)[0], k) == []:
        t = k*k
        eval(DtreeVf, root, string, pp, t)
...
def genPostOrderY(Dtree, root, k, string, pp):
    if Dtree.getChildrenID(Dtree.getChildrenID(root, k)[0], k) == []:
        t = k * k
        eval(Dtree, root, string, pp, t)
